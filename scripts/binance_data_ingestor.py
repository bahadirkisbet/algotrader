#!/usr/bin/env python3
"""
Binance Data Ingestor - Bulk Historical Data Download Script

This script is designed for ONE-TIME bulk downloads of historical data from Binance Vision.
It downloads multiple monthly files, validates them, and archives them locally.

NOTE: For on-demand data fetching during backtesting/analysis, the exchange module
(modules/exchange/exchange_library/binance/binance.py) has built-in Binance Vision support
that automatically fetches data as needed.

Use this script when:
- You want to pre-download large amounts of historical data
- You need to populate your local archive with historical data
- You want to validate and check data availability before processing

Use the exchange module's built-in support when:
- Running backtests (it fetches data automatically)
- Doing ad-hoc analysis
- You don't need to pre-download data
"""
...
class BinanceDataIngestor:
    ...
    async def fetch_monthly_data_links(
        self, symbol: str = "BTCUSDT", interval: str = "5m"
    ) -> List[str]:
    ...
    async def download_all_monthly_data(
        self, symbol: str = "BTCUSDT", interval: str = "5m"
    ) -> List[str]:
    ...
    def parse_csv_line(self, line: str, symbol: str) -> Candle:
        ...
        try:
            ...
            if (
                timestamp_seconds < 1262304000 or timestamp_seconds > 1893456000
            ):  # 2010-01-01 to 2030-01-01
                    ...
                    for line_num, line in enumerate(lines, 1):
                        ...
                        try:
                            ...
                            if invalid_lines <= 5:  # Only show first 5 errors to avoid spam
                                print(
                                    f"Warning: Failed to parse line {line_num} in {os.path.basename(zip_filepath)}: {e}"
                                )
                                if invalid_lines == 1:  # Show the problematic line for first error
                                    print(f"  Problematic line: {line[:100]}...")
                            elif invalid_lines == 6:
                                print(
                                    f"... and {invalid_lines - 5} more invalid lines in {os.path.basename(zip_filepath)}"
                                )
                            continue

                    if invalid_lines > 0:
                        print(
                            f"⚠️  {invalid_lines} invalid lines were skipped in {os.path.basename(zip_filepath)}"
                        )

                    print(
                        f"✓ Processed {zip_filepath}: {valid_candles} valid candles, {invalid_lines} invalid lines skipped"
                    )
